...
A subscription product is a standard WooCommerce product with the **Subscription [AS]** checkbox enabled. When a customer purchases it, the system creates a recurring subscription instead of a one-time order.
...
| Module | What It Adds |
|---|---|
| [Login Limit](../member-access/login-limit.md) | Concurrent session limits and plan-level session overrides |
