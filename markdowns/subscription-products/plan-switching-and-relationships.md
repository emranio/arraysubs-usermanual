...
Plan switching paths are configured on the **Linked Products** tab in the WooCommerce product editor. When a product has the **Subscription [AS]** checkbox enabled, a **Subscription Plan Switching** section appears in this tab.
...
![Subscription Billings [AS] tab Fixed Period Membership fields — Use fixed end date checkbox, End date type dropdown, and Annual cutoff selectors](plan-switching.ASSETS/04-fixed-period-membership-fields-annotated.png)

The Fixed Period Membership fields appear in the **Subscription Billings [AS]** tab of the product editor, below the standard billing fields. Check **Use fixed end date** to enable the feature.
...
|---|---|---|
| Plan switching fields not visible on Linked Products tab | Product is not marked as a subscription | Check the Subscription [AS] checkbox on the product |
